...
<div class="help-wrapper">
    <h1>ПК Расширение STMS<br/>
        «“Задания в производство” ExtComp»</h1>
    ...
    <ul>
        ...
        <li>Для компонентов не «Из STMS» - в случае совпадения <span class="goto" data-ref="Поля таблицы компонентов">Партномера</span></li>
    </ul>
    ...
    <p>Чтобы произвести быстрый поиск по партномеру, требуется ввести часть партномера компонента в поле «быстрый поиск»
        соответствующей таблицы. Поиск произведется по точному совпадению части названия без учета регистра.</p>
    <p>Действие аналогичное фильтрации по колонке «партномер» с автоматическим применением фильтра после каждого
        введенного
        символа.</p>
    ...
    <p>В появившемся окне требуется написать новое значения партномера. В случае, если такой партномер будет найден в STMS, система автоматически предложит его выбрать, в этом случае, компонент будет считаться компонентом <span class="goto" data-ref="Из STMS">«Из STMS»</span></p>
    ...
    <table class="lighttable">
        ...
        <tr><td>Партномер</td><td>Название компонента.</td><td>Текст</td><td>Если компонент взят <span class="goto" data-ref="Из STMS">из STMS</span>, данное поле точно соответствует партномеру. В таком случае, редактировать его нельзя.</td></tr>
    ...
    <table class="lighttable">
        ...
        <tr><td>Добавлено</td><td>Дата добавления</td><td>Дата</td><td>Заполняется автоматически.</td></tr>
        <tr><td>Партномер</td><td>Название компонента.</td><td>Текст</td><td>Если компонент взят <span class="goto" data-ref="Из STMS">из STMS</span>, данное поле точно соответствует партномеру.</td></tr>
