    ...
    public function accessRules()
    {
        return array(
            array('allow',  // allow all users to perform 'index' and 'view' actions
                ...
                'users' => array(self::USER_ALL),
            ),
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('index', 'registryList','getTasksTree','getTask','update','delete'),
//                'users' => array(self::USER_LOGGED),
                'users' => array(self::USER_ALL),
            ),
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions' => array('admin', 'delete','generatesome'),
                'users' => array(self::USER_ADMIN),
            ),
...
//                    }
    ...
    {
        $taksid = Yii::app()->request->getPost('taskid',0);
        /** @var Tasks $taskModel */
        $taskModel = Tasks::model()->findByPk($taksid);
        if(is_null($taskModel)){
            $this->j(array('message' => 'Заказ не найден'), false);
        }
        $criteria = new CDbCriteria();
        $criteria->addCondition('id>:id');
        $criteria->params = array_merge($criteria->params,array(':id'=>$taskModel->id));
        $newerTaskModelCount = Tasks::model()->count($criteria);
        if($newerTaskModelCount>0){
            $this->j(array('message' => 'Существуют более новые заказы ('.$newerTaskModelCount.' шт). Удаление невозможно.'), false);
            return;
        }
        $name = $taskModel->name;
        $taskModel->delete();
        $this->j(array('message' => 'Заказ '.$name.' удален'), true);
    }
